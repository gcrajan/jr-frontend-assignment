  ...
  return (
    <div>
        <div className={musiclistStyle.searchBar}>
            <input type="text" value={searchInput} onChange={event=> setSearchInput(event.target.value)} placeholder='eg Justin Bieber'/>
        </div>
        <div className={musiclistStyle.musicListContainer}>
        {info && info
        .filter((data) => data.result.artist_names && data.result.artist_names.toLowerCase().includes(searchInput.toLowerCase()))
        .map((data)=>(
